...
parser.add_argument('--exp', type=str, default='samt_pcl/try0918_mask64', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    if memory:
        segment_queue = torch.randn(num_classes, memory_bank, dim)
        segment_queue = nn.functional.normalize(segment_queue, p=2, dim=2)
        segment_queue_ptr = torch.zeros(num_classes, dtype=torch.long)
        pixel_queue = torch.zeros(num_classes, memory_bank, dim)
        pixel_queue = nn.functional.normalize(pixel_queue, p=2, dim=2)
        pixel_queue_ptr = torch.zeros(num_classes, dtype=torch.long)
    ...
    def _dequeue_and_enqueue(keys, labels):
        ...
        for bs in range(batch_size):
            ...
            for lb in this_label_ids:
                ...
                segment_queue_ptr[lb] = (segment_queue_ptr[lb] + 1) % memory_bank
                ...
                if ptr + K >= memory_bank:
                    ...
                    pixel_queue_ptr[lb] = (pixel_queue_ptr[lb] + 1) % memory_bank
    ...
    logging.info("输出max_epoch:{}=max_iteration // len(trainloader) + 1".format(max_epoch))
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            loss = args.lamda * loss_seg_dice + contrast_loss + loss_sdm + loss_unsup
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
