...
def train(args, snapshot_path):
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            loss_sdm = 0
            loss_sdm += F.l1_loss(seg_soft, dis_to_mask1)
            loss_sdm += F.l1_loss(seg_soft2, dis_to_mask2)
    ...
    # 绘制损失和迭代次数关系曲线
    plt.plot(iterations, losses_data)
    plt.xlabel('Iteration')
    plt.ylabel('Loss')
    plt.title('Loss vs. Iteration')

    # 保存为图片文件
    filename = "loss_curve_l1.png"  # 指定保存的文件名和路径
...
if __name__ == "__main__":
    ...
    # 保存路径/model/数据集名称_网络名称_有标签数据量_模型名称
